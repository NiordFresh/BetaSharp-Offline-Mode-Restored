    ...
    StorageService storageService) : ObservableObject
{
    ...
    [RelayCommand]
    private async Task UseOfflineAsync()
    {
        string? username = OfflineUsername?.Trim();

        if (string.IsNullOrWhiteSpace(username))
        {
            await alertService.ShowAsync("Offline Mode", "Please enter a username.");
            return;
        }
        ...
        {
            Name = username,
            Face = $"https://mc-heads.net/avatar/{username}/8.png",
            Token = "-",
